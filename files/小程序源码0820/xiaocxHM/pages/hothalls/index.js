let formatTime=require('../../common/utils.js')
let wxRequest=require('../../common/wxRequest.js')
Page({
  //页面的初始数据 
  data: {
    hothalls:[],
    hothallsType:'',
    currentPage:1
  },
  //生命周期函数--监听页面加载
  onLoad: function (options) {},
  //生命周期函数--监听页面初次渲染完成
  onReady: function () {},
  //生命周期函数--监听页面显示
  onShow: function () {
    this.getHotHallList(this)
  },
  getHotHallList(that){
    wxRequest.hgGet({
      currentPage:that.data.currentPage,
      pageSize:8
    },'index/index/getHotHallList').then(res=>{
      if(res.status===1){
        let hothalls=res.result.hothalls
        let hothallsType=res.result.type
        for(let i=0;i<hothalls.length;i++){
          hothalls[i].createTime=formatTime.getDateDiff(hothalls[i].createTime)
        }
        that.setData({
          currentPage:++that.data.currentPage,
          hothalls:that.data.hothalls.concat(hothalls),
          hothallsType:hothallsType
        })
      }
    })
  },
  //生命周期函数--监听页面隐藏
  onHide: function () {},
  //生命周期函数--监听页面卸载
  onUnload: function () {},
  //页面相关事件处理函数--监听用户下拉动作
  onPullDownRefresh: function () {},
  //页面上拉触底事件的处理函数
  onReachBottom: function () {
    this.getHotHallList(this)
  },
  //用户点击右上角分享
  onShareAppMessage: function () {},
})